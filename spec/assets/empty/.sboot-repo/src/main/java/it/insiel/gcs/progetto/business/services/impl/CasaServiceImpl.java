...
public class CasaServiceImpl implements CasaService {
    ...
    public CasaDTO salva(CasaDTO model) {
		...
		@Override
		public List<CasaDTO> salvaLista(List<CasaDTO> models) {

			List<Casa> entities = new ArrayList<Casa>();
			for (CasaDTO model : models){
				Casa entity = model.convert();
				entities.add(entity);
			}

			List<CasaDTO> dtos = new ArrayList<CasaDTO>();
			for (Casa entity : repository.save(entities)){
				CasaDTO dto = new CasaDTO(entity);
				dtos.add(dto);
			}
